...
export default function makeIdApp({ providerId, api, Conversation, ResourcesView, SubagentsView, MemoryView, TerminalPanel, RateLimitsBar, Stats, rawTypeOf, sessionTabs, subagentAdapter = null, docsUrl, docsTitle, usageInfo }) {
  ...
  function App({ active: appActive = true, providers, scopes, onOpenHome, onOpenSession, pendingOpen, navigationTarget, openTargets, onConsumedPending, onNavigate }) {
    ...
    const refetchActive = useCallback(() => {
      const a = activeRef.current
      if (!a) return
      const r = rootRef.current, request = openSeq.current
      api.session(r, a.id).then((d) => {
        if (openSeq.current !== request || rootRef.current !== r || activeRef.current?.id !== a.id) return
        setSessionData(d)
        setError(null)
      }).catch(() => {})
    ...
    for (const t of terminalEntries) if (t.provider === providerId && t.id) runningTermKeys.add(`${t.root}|${t.id}`)
    ...
    const openSessionById = useCallback(
      (id, { view } = {}) => {
        ...
        setTab(v)
        tabRef.current = v
        const memo = paneMemo.current.get(`${r}|${id}`)
        if (memo?.data) {
        ...
        api
          .session(r, id)
          .then((d) => {
            if (openSeq.current !== my || rootRef.current !== r) return
            setSessionData(d)
          ...
          .catch((e) => { if (openSeq.current === my && rootRef.current === r) setError(e.message) })
      },
      [loadSessions, report]
    )
    ...
    useEffect(() => {
      ...
      if (!pendingOpen.id && (pendingOpen.draft || pendingOpen.kind === 'tmux' || pendingOpen.newConversation) && (pendingOpen.slug || pendingOpen.cwd)) {
        ...
        setTermDraft(
          pendingOpen.slug
            ? { launchId: pendingOpen.launchId, terminalKey: pendingOpen.terminalKey, root: pendingOpen.root, slug: pendingOpen.slug, cwd: pendingOpen.cwd || pendingOpen.slug, title: pendingOpen.title || 'New conversation' }
                ...
                return (
                  <div key={pane.key} className={isCur ? 'contents' : 'hidden'}>
                    <TerminalPanel {...p} paneKey={pane.key} isNew={!p.id} terminalKey={terminalOf(p)?.key || p.terminalKey}
                      ...
                      onOpenTool={(what) => api.open(p.root, p.id || null, what, p.cwd, p.slug)} />
                  </div>
                )
              })}
          </div>
